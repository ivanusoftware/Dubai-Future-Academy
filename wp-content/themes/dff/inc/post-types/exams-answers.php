...
/**
 * Add post type for exams answers.
 *
 * @since 1.0.0
 * @return void
 */
if (!function_exists('dff_register_exams_answers_post_type')) {
	function dff_register_exams_answers_post_type()
	{
		register_post_type(
			'exams_answers',
			array(
				'labels'             => [
					'name'          => __('Exams Answers', 'dff'),
					'singular_name' => __('Exams Answer', 'dff'),
				],
				'public'             => true,
				'has_archive'        => false,
				...
				'taxonomies'         => array('exams_answers-categories'),
			)
...
if (!function_exists('dff_register_exams_answers_taxonomy')) {
	/**
	 * Register the Category taxonomy.
	 */
	function dff_register_exams_answers_taxonomy()
	{
		$labels = array(
		...
		$args   = array(
			...
			'rewrite'           => array(
				'slug'       => 'exams_answers/categories',
				'with_front' => false,
			),
		);

		register_taxonomy('exams_answers-categories', array('exams_answers'), $args);
	}
}

add_action('init', 'dff_register_exams_answers_post_type');
add_action('init', 'dff_register_exams_answers_taxonomy');
