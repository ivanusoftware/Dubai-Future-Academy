...
if (!function_exists('dff_register_quizzes_answers_taxonomy')) {
	/**
	 * Register the Category taxonomy.
	 */
	function dff_register_quizzes_answers_taxonomy()
	{
		$labels = array(
		...
		$args   = array(
			...
			'rewrite'           => array(
				'slug'       => 'quizzes_answers/categories',
				'with_front' => false,
			),
		);

		register_taxonomy('quizzes_answers-categories', array('quizzes_answers'), $args);
	}
}

add_action('init', 'dff_register_quizzes_answers_post_type');
add_action('init', 'dff_register_quizzes_answers_taxonomy');
